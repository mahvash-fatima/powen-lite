...
if ( ! function_exists( 'powen_posts_navigation' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 *
 * @todo Remove this function when WordPress 4.3 is released.
 */
function powen_posts_navigation() {
 ...
 * @todo Remove this function when WordPress 4.3 is released.
 */
function powen_the_post_navigation() {
...
if ( ! function_exists( 'powen_posted_on' ) ) :
/**
...
if ( ! function_exists( 'powen_archive_title' ) ) :
/**
...
function powen_archive_title( $before = '', $after = '' ) {
	 ...
	 */
	$title = apply_filters( 'get_powen_archive_title', $title );
...
if ( ! function_exists( 'powen_archive_description' ) ) :
/**
 ...
 */
function powen_archive_description( $before = '', $after = '' ) {
	$description = apply_filters( 'get_powen_archive_description', term_description() );
 ...
 * @return bool
 */
if( ! function_exists( 'powen_categorized_blog' ) )
{
	function powen_categorized_blog() {
		if ( false === ( $all_the_cool_cats = get_transient( 'powen_categories' ) ) ) {
			...
			set_transient( 'powen_categories', $all_the_cool_cats );
		}

		if ( $all_the_cool_cats > 1 ) {
			// This blog has more than 1 category so powen_categorized_blog should return true.
			return true;
		} else {
			// This blog has only 1 category so powen_categorized_blog should return false.
			return false;
		}
	}
}
...
/**
 * Flush out the transients used in powen_categorized_blog.
 */
function powen_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'powen_categories' );
}
add_action( 'edit_category', 'powen_category_transient_flusher' );
add_action( 'save_post',     'powen_category_transient_flusher' );
