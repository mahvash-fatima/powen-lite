  ...
  powen_UI = {
    ...
    init_add: function() {
      ...
      $(document).on('click', '.option-tree-list-item-setting-add', function(e) {
        ...
        powen_UI.add(this,'list_item_setting');
      });
    },
    init_edit: function() {
      $(document).on('click', '.option-tree-setting-edit', function(e) {
        ...
        if ( $(this).parents().hasClass('option-tree-setting-body') ) {
          powen_UI.init_remove_active($(this),'child');
          powen_UI.init_hide_body($(this),'child');
        } else {
          powen_UI.init_remove_active($(this),'parent');
          powen_UI.init_hide_body($(this), 'parent');
        }
        $(this).toggleClass('active');
        powen_UI.init_hide_body($(this), 'toggle');
      });
    },
    init_remove: function() {
      $(document).on('click', '.option-tree-setting-remove', function(event) {
        ...
        if (agree) {
          ...
          powen_UI.remove(this);
          setTimeout( function() { 
            powen_UI.update_ids(list); 
          }, 200 );
        }
        return false;
      });
        ...
        $.ajax({
          ...
          complete: function( data ) {
            if ( type == 'choice' || type == 'list_item_setting' ) {
              powen_UI.init_remove_active(list,'child-add');
              powen_UI.init_hide_body(list,'child-add');
            } else {
              powen_UI.init_remove_active();
              powen_UI.init_hide_body();
            }
            ...
            powen_UI.init_sortable(listItem);
            powen_UI.init_select_wrapper(listItem);
            powen_UI.init_numeric_slider(listItem);
            powen_UI.parse_condition();
    ...
    update_id: function(e) {
      ...
      this.timer = setTimeout( function() {
        powen_UI.update_ids($(e).parents('ul'));
      }, 100);
      return true;
    },
    ...
    parse_condition: function() {
      $( '.format-settings[id^="setting_"][data-condition]' ).each(function() {
        ...
        var passed;
        var conditions = powen_UI.match_conditions( $( this ).data( 'condition' ) );
        var operator = ( $( this ).data( 'operator' ) || 'and' ).toLowerCase();

        $.each( conditions, function( index, condition ) {

          var target   = $( '#setting_' + condition.check );
          var targetEl = !! target.length && target.find( powen_UI.condition_objects() ).first();
    ...
    init_conditions: function() {
      ...
      $('.format-settings[id^="setting_"]').on( 'change.conditionals, keyup.conditionals', powen_UI.condition_objects(), function(e) {
        if (e.type === 'keyup') {
        ...
        powen_UI.load_editors();
      });
      powen_UI.parse_condition();
    },
    init_upload: function() {
      $(document).on('click', '.powen_upload_media', function() {
        ...
        if ( window.wp && wp.media ) {
          window.powen_media_frame = window.powen_media_frame || new wp.media.view.MediaFrame.Select({
          ...
          window.powen_media_frame.on('select', function() {
            var attachment = window.powen_media_frame.state().get('selection').first(), 
            ...
            window.powen_media_frame.off('select');
          }).open();
        } else {
          ...
          window.send_to_editor = function(html) {
            ...
            if (href.match(image) && powen_UI.url_exists(href)) {
              btnContent += '<div class="option-tree-ui-image-wrap"><img src="'+href+'" alt="" /></div>';
            }
            btnContent += '<a href="javascript:(void);" class="option-tree-ui-remove-media option-tree-ui-button button button-secondary light" title="'+option_tree.remove_media_text+'"><span class="icon ot-icon-minus-circle"></span>'+option_tree.remove_media_text+'</a>';
            $('#'+field_id).val(href);
    ...
    init_upload_fix: function(elm) {
      ...
      if ( val !== '' && ( typeof src == 'undefined' || src == false ) && powen_UI.url_exists(val) ) {
        var image = /\.(?:jpe?g|png|gif|ico)$/i;
        if (val.match(image)) {
      ...
      } else if ( val == '' || ! powen_UI.url_exists(val) ) {
    ...
    load_editors: function() {
      powen_UI.css_editor_mode();
      powen_UI.javascript_editor_mode();
    },
...
!function ($) {
  ...
  powen_gallery = {
    ...
    frame: function (elm) {
      ...
      this._frame.on('update', function () {
        var controller = powen_gallery._frame.states.get('gallery-edit')
  ...
  , select: function (elm) {
      var input = $(elm).parents('.format-setting-inner').children('.ot-gallery-value')
        ...
        , shortcode = wp.shortcode.next('gallery', ( ids ? _shortcode : wp.media.view.settings.powen_gallery.shortcode ) )
  ...
  // Gallery delete
  $(document).on('click.powen_gallery.data-api', '.ot-gallery-delete', function (e) {
    e.preventDefault()
    powen_gallery.remove($(this))
  })
  
  // Gallery edit
  $(document).on('click.powen_gallery.data-api', '.ot-gallery-edit', function (e) {
    e.preventDefault()
    powen_gallery.open($(this))
  })
      ...
      $('.wrap').on('mouseup', function () {
        ...
        $('#' + parent_id + ' input[type="radio"]').each( function () {
          ...
          if ( $(this).is(':checked') ) {
            ...
            powen_checked_radios[$(this).attr('name')] = $(this).val()
          
          }
          
          // write to the object
          $(document).data('powen_checked_radios', powen_checked_radios)
        ...
        setTimeout( function () {
          ...
          var checked = $(document).data('powen_checked_radios')
...
;(function ( $, window, document, undefined ) {
  ...
  $.powen_ParseColorValue = function( val ) {
  ...
  $.fn.powen_wpColorPicker = function() {
    ...
    return this.each(function() {
      ...
      if( $this.data('rgba') !== false ) {
        ...
        // parse value
        var picker = $.powen_ParseColorValue( $this.val() );
            ...
            $alpha_slider.slider({
              ...
              create: function() {
                ...
                $container.on('click', '.wp-picker-default', function() {
                  ...
                  var default_picker = $.powen_ParseColorValue( $this.data('default-color') ),
  ...
  $(document).ready( function(){
    $('.hide-color-picker.ot-colorpicker-opacity').powen_wpColorPicker();
  });
