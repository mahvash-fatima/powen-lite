...
* Hotfix - Force the `powen_google_fonts` array to be rebuilt when switching between themes.
...
* Added filter `powen_dequeue_jquery_ui_css_screen_ids` to dequeue `jquery-ui-css` by screen ID.
* Added filter `powen_on_off_switch_on_value` to filter the value of the On button. Contributors via github @BassemN, and @valendesigns.
* Added filter `powen_on_off_switch_on_label` to filter the label of the On button. Contributors via github @BassemN, and @valendesigns.
* Added filter `powen_on_off_switch_off_value` to filter the value of the Off button. Contributors via github @BassemN, and @valendesigns.
* Added filter `powen_on_off_switch_off_label` to filter the label of the Off button. Contributors via github @BassemN, and @valendesigns.
* Added filter `powen_on_off_switch_width` to filter the width of the On/Off switch.
* Added filter `powen_type_date_picker_readonly` to filter the addition of the readonly attribute.
* Added filter `powen_type_date_time_picker_readonly` to filter the addition of the readonly attribute.
* Added filter `powen_admin_menu_priority` to filter the `admin_menu` action hook priority.
...
* Changed where `powen_css_file_paths` is saved when `is_multisite` for better `dynamic.css` file support.
...
* Hotfix - Fixed an issue where `powen_get_media_post_ID()` was never able to set the value of the `powen_media_post_ID` option because it was already set to empty. Causing the `powen_create_media_post()` function to create multiple media posts.

= 2.4.5 =
* Hotfix - Fixed an issue where `powen_get_media_post_ID()` was setting the value of the `powen_media_post_ID` option to `null`. Causing the `powen_create_media_post()` function to create multiple media posts. A clean up script will be added to `2.5.0`.
...
* Hotfix - Fixed the `powen_line_height_range_interval` filter being misnamed as `powen_line_height_unit_type`. Contributors via github @youri--.
...
* Hotfix - Check for `post_title` instead of `post_name` in `powen_get_media_post_ID()`. Contributors via github @clifgriffin.
* Hotfix - Store the return value of `powen_get_media_post_ID()` in the options table as `powen_media_post_ID`.
...
* Hotfix - Added filter `powen_override_forced_textarea_simple` to allow the Textarea option type to be moved in the DOM and not replaced with the Textarea Simple option type in meta boxes and list items.
...
= 2.4.0 =
* Added filter 'powen_post_formats' which loads meta boxes specifically for post formats.
...
* Added filter 'powen_type_select_choices' to dynamically change select choices. Contributors via github @maimairel and @valendesigns.
...
* Added filter `powen_recognized_font_sizes` to dynamically change the font sizes by field ID.
* Added filter `powen_recognized_letter_spacing` to dynamically change the letter spacing by field ID.
* Added filter `powen_recognized_line_heights` to dynamically change the line heights by field ID.
...
* Hotfix - Added filter `powen_migrate_settings_id` to migrate themes that used `option_tree_settings` and now use a custom settings ID.
* Hotfix - Added filter `powen_migrate_options_id` to migrate themes that used `option_tree` and now use a custom options ID.
* Hotfix - Added filter `powen_migrate_layouts_id` to migrate themes that used `option_tree_layouts` and now use a custom layouts ID.

= 2.3.2 =
* Hotfix - Fixed an issue with the `powen_create_media_post` function creating multiple `option-tree` posts.
...
* Hotfix - Added filter `powen_type_date_picker_date_format` to change the date format of the Date Picker option type.
* Hotfix - Added filter `powen_type_date_time_picker_date_format` to change the date format of the Date Time Picker option type.
...
* Added filter 'powen_list_item_title_label' to change the label for a List Item's required title field.
* Added filter 'powen_list_item_title_desc' to change the description for a List Item's required title field.
* Added filter 'powen_options_id' to change the 'option_tree' option ID to a unique value.
* Added filter 'powen_settings_id' to change the 'option_tree_settings' option ID to a unique value.
* Added filter 'powen_layouts_id' to change the 'option_tree_layouts' option ID to a unique value.
* Added filter 'powen_header_logo_link' to change the logo link inside the header of OptionTree.
* Added filter 'powen_header_version_text' to change the version text inside the header of OptionTree.
* Added action 'powen_header_list' to add additional theme specific list items to the header of OptionTree.
* Added filter 'powen_upload_text' to change the "Send to OptionTree" text.
...
* Fixed a bug that kept the UI from displaying when using the `powen_type_background_size_choices` filter.
...
* Added filter 'powen_type_radio_image_src' which allows the Radio Image option type source URI to be changed. Contributors via github @bitcommit.
...
* Added filter 'powen_recognized_background_fields' to show/hide fields for background option types.
* Added filter 'powen_filter_description' that allows the theme option descriptions to be filtered before being displayed.
...
* Added filter 'powen_type_category_checkbox_query' which allows you to filter the get_categories() args for Category Checkbox.
* Added filter 'powen_type_category_select_query' which allows you to filter the get_categories() args for Category Select.
* Added filter 'powen_type_taxonomy_checkbox_query' which allows you to filter the get_categories() args for Taxonomy Checkbox.
* Added filter 'powen_type_taxonomy_select_query' which allows you to filter the get_categories() args for Taxonomy Select.
* Added the 'powen_echo_option' function. Contributors via github @joshlevinson.
* Added filter 'powen_theme_options_contextual_help' which allows you to filter the Contextual Help on the Theme Options page.
* Added filter 'powen_theme_options_sections' which allows you to filter the Sections on the Theme Options page. Contributors via github @joshlevinson.
* Added filter 'powen_theme_options_settings' which allows you to filter the Settings on the Theme Options page. Contributors via github @joshlevinson.
...
= 2.1.1 =
* Hotfix - The powen_SHOW_SETTINGS_EXPORT constant was incorrectly set to false as the default.
...
* Added dynamic replacement of 'powen_URL' & 'powen_THEME_URL' in the Radio Image source path.
* Make '0' possible as a field value. Validate for empty strings instead of empty(). Contributors via github @maimairel.
* The 'powen_theme_options_capability' filter is now working for different capabilities like editor.
* The 'powen_display_by_type' filter is now being assigned to a value.
* Added filter 'powen_show_options_ui' which allows you to hide the Theme Options UI Builder.
* Added filter 'powen_show_settings_import' which allows you to hide the Settings Import options on the Import page.
* Added filter 'powen_show_settings_export' which allows you to hide the Settings Export options on the Export page.
* Added filter 'powen_show_docs' which allows you to hide the Documentation.
* Added filter 'powen_use_theme_options' which allows you to hide the OptionTree Theme Option page (not recommended for beginners).
* Added filter 'powen_list_item_description' which allows you to change the default list item description text.
* Added filter 'powen_type_custom_post_type_checkbox_query' which allows you to filter the get_posts() args for Custom Post Type Checkbox.
* Added filter 'powen_type_custom_post_type_select_query' which allows you to filter the get_posts() args for Custom Post Type Select.
* Added filter 'powen_type_page_checkbox_query' which allows you to filter the get_posts() args for Page Checkbox.
* Added filter 'powen_type_page_select_query' which allows you to filter the get_posts() args for Page Select.
* Added filter 'powen_type_post_checkbox_query' which allows you to filter the get_posts() args for Post Checkbox.
* Added filter 'powen_type_post_select_query' which allows you to filter the get_posts() args for Post Select.
...
* Added 'powen_before_theme_options_save' action hook.
...
* Fixed missing required arguments in powen_Loader::add_layout.
* Removed esc_attr() on font-family check.
* Added a 'powen_theme_options_parent_slug' filter in ot-ui-theme-options.php
* Fixed WP_Error from the use of wp_get_remote() instead of file_get_contents().

= 2.0.13 =
* Removed almost all of the Theme Check nag messages when in 'powen_theme_mode'.
...
* Added the 'powen_after_validate_setting' filter to the validation function.
* Added $field_id to the powen_validate_setting() for more precise filtering.
* Added the powen_reverse_wpautop() function that you can run input through just incase you need it.
...
* Added the simpler powen_get_option() function to eventually replace get_option_tree().
