<?php if ( ! defined( 'powen_VERSION' ) ) exit( 'No direct script access allowed' );
/**
 ...
 * @since     2.1
 */
if ( ! function_exists( 'powen_register_theme_options_page' ) ) {

  function powen_register_theme_options_page() {
  
    /* get the settings array */
    $get_settings = get_option( powen_settings_id() );
    ...
    if ( function_exists( 'powen_register_settings' ) && powen_USE_THEME_OPTIONS ) {
      
      powen_register_settings( array(
          array(
            'id'                  => powen_options_id(),
            'pages'               => array( 
              array(
                'id'              => 'powen_theme_options',
                'parent_slug'     => apply_filters( 'powen_theme_options_parent_slug', 'themes.php' ),
                'page_title'      => apply_filters( 'powen_theme_options_page_title', __( 'Theme Options', 'option-tree' ) ),
                'menu_title'      => apply_filters( 'powen_theme_options_menu_title', __( 'Theme Options', 'option-tree' ) ),
                'capability'      => $caps = apply_filters( 'powen_theme_options_capability', 'edit_theme_options' ),
                'menu_slug'       => apply_filters( 'powen_theme_options_menu_slug', 'powen-theme-options' ),
                'icon_url'        => apply_filters( 'powen_theme_options_icon_url', null ),
                'position'        => apply_filters( 'powen_theme_options_position', null ),
                'updated_message' => apply_filters( 'powen_theme_options_updated_message', __( 'Theme Options updated.', 'option-tree' ) ),
                'reset_message'   => apply_filters( 'powen_theme_options_reset_message', __( 'Theme Options reset.', 'option-tree' ) ),
                'button_text'     => apply_filters( 'powen_theme_options_button_text', __( 'Save Changes', 'option-tree' ) ),
                'contextual_help' => apply_filters( 'powen_theme_options_contextual_help', $contextual_help ),
                'sections'        => apply_filters( 'powen_theme_options_sections', $sections ),
                'settings'        => apply_filters( 'powen_theme_options_settings', $settings )
              )
            )
          )
        ) 
      ...
      add_filter( 'option_page_capability_' . powen_options_id(), create_function( '$caps', "return '$caps';" ), 999 );
 ...
 * @since     2.1
 */
if ( ! function_exists( 'powen_register_settings_page' ) ) {

  function powen_register_settings_page() {
    global $powen_has_custom_theme_options;
    
    // Display UI Builder admin notice
    if ( powen_SHOW_OPTIONS_UI == true && isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'ot-settings' && ( $powen_has_custom_theme_options == true || has_action( 'admin_init', 'custom_theme_options' ) || has_action( 'init', 'custom_theme_options' ) ) ) {
      
      function powen_has_custom_theme_options() {
      ...
      add_action( 'admin_notices', 'powen_has_custom_theme_options' );
  
    }
    
    // Create the filterable pages array
    $powen_register_pages_array =  array( 
      array( 
        'id'              => 'ot',
      ...
      array(
        ...
        'settings'        => array(
          ...
          array(
            'id'          => 'functions_powen_get_option',
            'label'       => __( 'Function Reference:powen_get_option()', 'option-tree' ),
            'type'        => 'ot-get-option',
            'section'     => 'functions'
          ),
          array(
    ...
    foreach( $powen_register_pages_array as $key => $page ) {
      ...
      if ( $page['id'] == 'settings' ) {
        ...
        if ( powen_SHOW_SETTINGS_IMPORT == false ) {
        ...
        if ( powen_SHOW_SETTINGS_EXPORT == false ) {
        ...
        if ( powen_SHOW_NEW_LAYOUT == false ) {
      ...
      if ( powen_SHOW_DOCS == false && $page['id'] == 'documentation' ) {
        
        unset( $powen_register_pages_array[$key] );
      
      }
    
    }
    
    $powen_register_pages_array = apply_filters( 'powen_register_pages_array', $powen_register_pages_array );
    
    // Register the pages.
    powen_register_settings( array(
        array(
          'id'              => powen_settings_id(),
          'pages'           => $powen_register_pages_array
        )
      )
    );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_after_theme_options_save' ) ) {

  function powen_after_theme_options_save() {
    ...
    if ( apply_filters( 'powen_theme_options_menu_slug', 'powen-theme-options' ) == $page && $updated ) {
      
      /* grab a copy of the theme options */
      $options = get_option( powen_options_id() );
      
      /* execute the action hook and pass the theme options to it */
      do_action( 'powen_after_theme_options_save', $options );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_validate_setting' ) ) {

  function powen_validate_setting( $input, $type, $field_id, $wmpl_id = '' ) {
    
    /* exit early if missing data */
    if ( ! $input || ! $type || ! $field_id )
      return $input;
    
    $input = apply_filters( 'powen_validate_setting', $input, $type, $field_id );
    
    /* WPML Register and Unregister strings */
    if ( ! empty( $wmpl_id ) ) {
    
      /* Allow filtering on the WPML option types */
      $single_string_types = apply_filters( 'powen_wpml_option_types', array( 'text', 'textarea', 'textarea-simple' ) );
              
      if ( in_array( $type, $single_string_types ) ) {
      
        if ( ! empty( $input ) ) {
        
          powen_wpml_register_string( $wmpl_id, $input );
          
        } else {
        
          powen_wpml_unregister_string( $wmpl_id );
    ...
    if ( 'background' == $type ) {
      ...
      $input['background-color'] = powen_validate_setting( $input['background-color'], 'colorpicker', $field_id );
      
      $input['background-image'] = powen_validate_setting( $input['background-image'], 'upload', $field_id );
      ...
      foreach( $input as $key => $value ) {
        ...
        if ( $key == 'color' && ! empty( $value ) ) {
          ...
          $input[$key] = powen_validate_setting( $value, 'colorpicker', $field_id );
      ...
      // Validate offset-x
      $input['offset-x'] = powen_validate_setting( $input['offset-x'], 'text', $field_id );
      
      // Validate offset-y
      $input['offset-y'] = powen_validate_setting( $input['offset-y'], 'text', $field_id );
      
      // Validate blur-radius
      $input['blur-radius'] = powen_validate_setting( $input['blur-radius'], 'text', $field_id );
      
      // Validate spread-radius
      $input['spread-radius'] = powen_validate_setting( $input['spread-radius'], 'text', $field_id );
      
      // Validate color
      $input['color'] = powen_validate_setting( $input['color'], 'colorpicker', $field_id );
      ...
      // Validate color
      $input = powen_validate_setting( $input, 'colorpicker', $field_id );

    } else if ( in_array( $type, array( 'css', 'javascript', 'text', 'textarea', 'textarea-simple' ) ) ) {
      
      if ( ! current_user_can( 'unfiltered_html' ) && powen_ALLOW_UNFILTERED_HTML == false ) {
      ...
      if ( is_array( $input ) && ! empty( $input ) ) {
        foreach( $input as $key => $value ) {
          if ( ! empty( $value ) ) {
            $input[$key] = powen_validate_setting( $input[$key], 'colorpicker', $field_id . '-' . $key );
            $has_value = true;
          }
        }
      }
      ...
      $input['font-color'] = powen_validate_setting( $input['font-color'], 'colorpicker', $field_id );
    ...
    $input = apply_filters( 'powen_after_validate_setting', $input, $type, $field_id );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_admin_styles' ) ) {

  function powen_admin_styles() {
    global $wp_styles, $post;
    
    /* execute styles before actions */
    do_action( 'powen_admin_styles_before' );
    
    /* load WP colorpicker */
    wp_enqueue_style( 'wp-color-picker' );
    
    /* load admin styles */
    wp_enqueue_style( 'ot-admin-css', powen_URL . 'assets/css/ot-admin.css', false, powen_VERSION );
     ...
     */
    $screen_ids = apply_filters( 'powen_dequeue_jquery_ui_css_screen_ids', array( 
      'toplevel_page_ot-settings', 
      'optiontree_page_ot-documentation', 
      'appearance_page_powen-theme-options' 
    ) );
    ...
    do_action( 'powen_admin_styles_after' );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_admin_scripts' ) ) {

  function powen_admin_scripts() {
    
    /* execute scripts before actions */
    do_action( 'powen_admin_scripts_before' );
    ...
    wp_enqueue_script( 'jquery-ui-timepicker', powen_URL . 'assets/js/vendor/jquery/jquery-ui-timepicker.js', array( 'jquery', 'jquery-ui-slider', 'jquery-ui-datepicker' ), '1.4.3' );

    /* load the post formats */
    if ( powen_META_BOXES == true && powen_POST_FORMATS == true ) {
      wp_enqueue_script( 'ot-postformats', powen_URL . 'assets/js/ot-postformats.js', array( 'jquery' ), '1.0.1' );
    }

    /* load all the required scripts */
    wp_enqueue_script( 'ot-admin-js', powen_URL . 'assets/js/ot-admin.js', array( 'jquery', 'jquery-ui-tabs', 'jquery-ui-sortable', 'jquery-ui-slider', 'wp-color-picker', 'ace-editor', 'jquery-ui-datepicker', 'jquery-ui-timepicker' ), powen_VERSION );

    /* create localized JS array */
    $localized_array = array( 
      'ajax'                  => admin_url( 'admin-ajax.php' ),
      'upload_text'           => apply_filters( 'powen_upload_text', __( 'Send to OptionTree', 'option-tree' ) ),
    ...
    do_action( 'powen_admin_scripts_after' );
 ...
 * @since       2.0
 */
if ( ! function_exists( 'powen_get_media_post_ID' ) ) {

  function powen_get_media_post_ID() {
    
    // Option ID
    $option_id = 'powen_media_post_ID';
 ...
 * @since       2.0
 */
if ( ! function_exists( 'powen_create_media_post' ) ) {
  
  function powen_create_media_post() {
    ...
    $post_id = powen_get_media_post_ID();
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_default_settings' ) ) {

  function powen_default_settings() {
    global $wpdb, $table_prefix;
    
    if ( ! get_option( powen_settings_id() ) ) {
      ...
      if ( count( $wpdb->get_results( "SHOW TABLES LIKE '{$table_prefix}option_tree'" ) ) == 1 && $old_settings = $wpdb->get_results( "SELECT * FROM {$table_prefix}option_tree ORDER BY item_sort ASC" ) ) {
        ...
        foreach ( $old_settings as $setting ) {
          ...
          if ( $setting->item_type == 'heading' ) {
            ...
            $settings['settings'][$settings_count]['type'] = powen_map_old_option_types( $setting->item_type );
            ...
            if ( in_array( $settings['settings'][$settings_count]['type'], array( 'checkbox', 'radio', 'select' ) ) ) {
              if ( '' != $setting->item_options ) {
                $choices = powen_convert_string_to_array( $setting->item_options );
              }
            }
      ...
      update_option( powen_settings_id(), $settings );
      ...
      if ( is_array( $options ) ) {
        ...
        foreach( $settings['settings'] as $setting ) {
          ...
          if ( isset( $options[$setting['id']] ) ) {
            ...
            $content = powen_stripslashes( $options[$setting['id']] );
            
            $options[$setting['id']] = powen_validate_setting( $content, $setting['type'], $setting['id'] );
        ...
        /* execute the action hook and pass the theme options to it */
        do_action( 'powen_before_theme_options_save', $options );
        
        /* update the option tree array */
        update_option( powen_options_id(), $options );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_save_css' ) ) {

  function powen_save_css( $options ) {
    
    /* grab a copy of the settings */
    $settings = get_option( powen_settings_id() );
      ...
      foreach( $settings['settings'] as $k => $setting ) {
        ...
        if ( isset( $setting['type'] ) && 'css' == $setting['type'] ) {
          ...
          if ( isset( $options[$setting['id']] ) && '' !== $options[$setting['id']] ) {
            ...
            powen_insert_css_with_markers( $setting['id'], $options[$setting['id']] );
          
          /* remove old CSS from dynamic.css */
          } else {
          
            powen_remove_old_css( $setting['id'] );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_add_xml_to_upload_filetypes' ) ) {

  function powen_add_xml_to_upload_filetypes() {
    
    add_filter( 'upload_mimes', 'powen_upload_mimes' );
    add_filter( 'wp_mime_type_icon', 'powen_xml_mime_type_icon', 10, 2 );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_upload_mimes' ) ) {

  function powen_upload_mimes( $mimes ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_xml_mime_type_icon' ) ) {

  function powen_xml_mime_type_icon( $icon, $mime ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_import' ) ) {

  function powen_import() {
    ...
    if ( isset( $_POST['import_xml_nonce'] ) && wp_verify_nonce( $_POST['import_xml_nonce'], 'import_xml_form' ) ) {
      ...
      if ( preg_match( "/(.xml)$/i", $file ) && class_exists( 'SimpleXMLElement' ) ) {
        ...
        $settings = powen_import_xml( $file );
    ...
    if ( isset( $_POST['import_settings_nonce'] ) && wp_verify_nonce( $_POST['import_settings_nonce'], 'import_settings_form' ) ) {
      ...
      /* textarea value */
      $textarea = isset( $_POST['import_settings'] ) ? unserialize( powen_decode( $_POST['import_settings'] ) ) : '';
      ...
      if ( is_array( $textarea ) ) {
        update_option( powen_settings_id(), $textarea );
        $message = 'success';
      }
    ...
    if ( isset( $_POST['import_data_nonce'] ) && wp_verify_nonce( $_POST['import_data_nonce'], 'import_data_form' ) ) {
      ...
      /* textarea value */
      $options = isset( $_POST['import_data'] ) ? unserialize( powen_decode( $_POST['import_data'] ) ) : '';
    ...
    if ( isset( $_POST['import_layouts_nonce'] ) && wp_verify_nonce( $_POST['import_layouts_nonce'], 'import_layouts_form' ) ) {
      ...
      /* textarea value */
      $layouts = isset( $_POST['import_layouts'] ) ? unserialize( powen_decode( $_POST['import_layouts'] ) ) : '';
      ...
      if ( is_array( $layouts ) ) {
        ...
        if ( is_array( $settings ) ) {
          ...
          foreach( $layouts as $key => $value ) {
            ...
            $options = unserialize( powen_decode( $value ) );
            ...
            $layouts[$key] = powen_encode( serialize( $options ) );
        ...
        if ( isset( $layouts['active_layout'] ) ) {
          ...
          $new_options = unserialize( powen_decode( $layouts[$layouts['active_layout']] ) );
          
          /* execute the action hook and pass the theme options to it */
          do_action( 'powen_before_theme_options_save', $new_options );
        
          update_option( powen_options_id(), $new_options );
          
        }
        
        /* update the option tree layouts array */
        update_option( powen_layouts_id(), $layouts );
 ...
 * @since     2.0.8
 */
if ( ! function_exists( 'powen_export' ) ) {

  function powen_export() {
    ...
    if ( isset( $_POST['export_settings_file_nonce'] ) && wp_verify_nonce( $_POST['export_settings_file_nonce'], 'export_settings_file_form' ) ) {
      ...
      powen_export_php_settings_array();
 ...
 * @since     2.0.8
 */
if ( ! function_exists( 'powen_import_xml' ) ) {

  function powen_import_xml( $file ) {
    ...
    if ( $rawdata ) {
      ...
      foreach ( $xml->row as $value ) {
        ...
        if ( $value->item_type == 'heading' ) {
          ...
          $settings['settings'][$settings_count]['section'] = $section;
          $settings['settings'][$settings_count]['type'] = powen_map_old_option_types( (string) $value->item_type );
          $settings['settings'][$settings_count]['std'] = '';
          $settings['settings'][$settings_count]['class'] = '';
          ...
          if ( in_array( $settings['settings'][$settings_count]['type'], array( 'checkbox', 'radio', 'select' ) ) ) {
            if ( '' != (string) $value->item_options ) {
              $choices = powen_convert_string_to_array( (string) $value->item_options );
            }
          }
          $settings['settings'][$settings_count]['choices'] = $choices;
 ...
 * @since     2.0.8
 */
if ( ! function_exists( 'powen_export_php_settings_array' ) ) {

  function powen_export_php_settings_array() {
    ...
    $sections             = '';
    $settings             = '';
    $option_tree_settings = get_option( powen_settings_id(), array() );
    
    // Domain string helper
    function powen_I18n_string( $string ) {
    ...
    if ( isset( $option_tree_settings['contextual_help']['content'] ) ) {
      ...
      foreach( $option_tree_settings['contextual_help']['content'] as $value ) {
        ...
        $_title = powen_I18n_string( isset( $value['title'] ) ? str_replace( "'", "\'", $value['title'] ) : '' );
        $_content = powen_I18n_string( isset( $value['content'] ) ? html_entity_decode(  str_replace( "'", "\'", $value['content'] ) ) : '' );
        $help.= "
        array(
          'id'        => '$_id',
          'title'     => $_title,
    ...
    if ( isset( $option_tree_settings['contextual_help']['sidebar'] ) ) {
      $contextual_help.= "
      'sidebar'       => " . powen_I18n_string( html_entity_decode(  str_replace( "'", "\'", $option_tree_settings['contextual_help']['sidebar'] ) ) );
    }
    ...
    if ( isset( $option_tree_settings['settings'] ) ) {
      foreach( $option_tree_settings['settings'] as $value ) {
        $_id = isset( $value['id'] ) ? $value['id'] : '';
        $_label = powen_I18n_string( isset( $value['label'] ) ? str_replace( "'", "\'", $value['label'] ) : '' );
        $_desc = powen_I18n_string( isset( $value['desc'] ) ? str_replace( "'", "\'", $value['desc'] ) : '' );
        ...
        if ( isset( $value['choices'] ) && ! empty( $value['choices'] ) ) {
          foreach( $value['choices'] as $choice ) {
            ...
            $_choice_label = powen_I18n_string( isset( $choice['label'] ) ? str_replace( "'", "\'", $choice['label'] ) : '' );
...
' . implode( ",\n", $std_array ) . '
        ...
        if ( isset( $value['settings'] ) && ! empty( $value['settings'] ) ) {
          foreach( $value['settings'] as $setting ) {
            ...
            $_setting_label = powen_I18n_string( isset( $setting['label'] ) ? str_replace( "'", "\'", $setting['label'] ) : '' );
            $_setting_desc = powen_I18n_string( isset( $setting['desc'] ) ? str_replace( "'", "\'", $setting['desc'] ) : '' );
            ...
            if ( isset( $setting['choices'] ) && ! empty( $setting['choices'] ) ) {
              foreach( $setting['choices'] as $setting_choice ) {
                ...
                $_setting_choice_label = powen_I18n_string( isset( $setting_choice['label'] ) ? str_replace( "'", "\'", $setting_choice['label'] ) : '' );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_save_settings' ) ) {

  function powen_save_settings() {

    /* check and verify import settings nonce */
    if ( isset( $_POST['option_tree_settings_nonce'] ) && wp_verify_nonce( $_POST['option_tree_settings_nonce'], 'option_tree_settings_form' ) ) {

      /* settings value */
      $settings = isset( $_POST[powen_settings_id()] ) ? $_POST[powen_settings_id()] : '';
      ...
      if ( isset( $settings['sections'] ) ) {
        ...
        foreach( $settings['sections'] as $k => $section ) {
          ...
          if ( ( ! isset( $section['title'] ) && ! isset( $section['id'] ) ) || ( '' == $section['title'] && '' == $section['id'] ) ) {
            ...
            $settings['sections'][$k]['id'] = powen_sanitize_option_id( wp_kses_post( $section['id'] ) );
            
          }
          
        }
        
        $settings['sections'] = powen_stripslashes( $settings['sections'] );
      ...
      if ( isset( $settings['settings'] ) ) {
        ...
        $settings['settings'] = powen_validate_settings_array( $settings['settings'] );
      ...
      if ( isset( $settings['contextual_help']['content'] ) ) {
        ...
        foreach( $settings['contextual_help']['content'] as $k => $content ) {
          ...
          if ( ( ! isset( $content['title'] ) && ! isset( $content['id'] ) ) || ( '' == $content['title'] && '' == $content['id'] ) ) {
            ...
            /* sanitize ID once everything has been checked first */
            $settings['contextual_help']['content'][$k]['id'] = powen_sanitize_option_id( wp_kses_post( $content['id'] ) );
      ...
      $settings['contextual_help'] = powen_stripslashes( $settings['contextual_help'] );
      ...
      if ( is_array( $settings ) ) {
        ...
        if ( function_exists( 'icl_unregister_string' ) ) {
          ...
          $current = get_option( powen_settings_id() );
          $options = get_option( powen_options_id() );
                ...
                if ( ! empty( $options[$current_setting['id']] ) && in_array( $current_setting['type'], array( 'list-item', 'slider' ) ) ) {
                  ...
                  foreach( $options[$current_setting['id']] as $key => $value ) {
                    ...
                    foreach( $value as $ckey => $cvalue ) {
                      ...
                      powen_wpml_unregister_string( $current_setting['id'] . '_' . $ckey . '_' . $key );
                  ...
                  powen_wpml_unregister_string( $current_setting['id'] );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_validate_settings_array' ) ) {

  function powen_validate_settings_array( $settings = array() ) {
    ...
    if ( count( $settings ) > 0 ) {
      ...
      foreach( $settings as $k => $setting ) {
        ...
        if ( ( ! isset( $setting['label'] ) && ! isset( $setting['id'] ) ) || ( '' == $setting['label'] && '' == $setting['id'] ) ) {
          ...
          /* sanitize ID once everything has been checked first */
          $settings[$k]['id'] = powen_sanitize_option_id( wp_kses_post( $setting['id'] ) );
          ...
          foreach( $setting['choices'] as $ck => $choice ) {
            ...
            if ( ( ! isset( $choice['label'] ) && ! isset( $choice['value'] ) ) || ( '' == $choice['label'] && '' == $choice['value'] ) ) {
              ...
              /* missing value set to label */ 
                ...
                $setting['choices'][$ck]['value'] = powen_sanitize_option_id( wp_kses_post( $choice['label'] ) );
        ...
        if ( isset( $setting['settings'] ) ) {
          ...
          $settings[$k]['settings'] = powen_validate_settings_array( $setting['settings'] );
    ...
    return powen_stripslashes( $settings );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_modify_layouts' ) ) {

  function powen_modify_layouts() {
    ...
    if ( isset( $_POST['option_tree_modify_layouts_nonce'] ) && wp_verify_nonce( $_POST['option_tree_modify_layouts_nonce'], 'option_tree_modify_layouts_form' ) ) {
      ...
      $option_tree_layouts = get_option( powen_layouts_id() );
      
      /* new layouts value */
      $layouts = isset( $_POST[powen_layouts_id()] ) ? $_POST[powen_layouts_id()] : '';
      ...
      if ( is_array( $layouts ) && ! empty( $layouts ) ) {
        ...
        if ( isset( $layouts['_add_new_layout_'] ) && ! empty( $layouts['_add_new_layout_'] ) ) {
          $rebuild['active_layout'] = powen_sanitize_layout_id( $layouts['_add_new_layout_'] );
          $rebuild[$rebuild['active_layout']] = powen_encode( serialize( get_option( powen_options_id() ) ) );
        }
      ...
      if ( count( $rebuild ) > 1 ) {
        ...
        $rebuild_option_tree = unserialize( powen_decode( $rebuild[$rebuild['active_layout']] ) );
        if ( is_array( $rebuild_option_tree ) ) {
          
          /* execute the action hook and pass the theme options to it */
          do_action( 'powen_before_theme_options_save', $rebuild_option_tree );
          
          update_option( powen_options_id(), $rebuild_option_tree );
          
        }
        
        /* rebuild the layouts */
        update_option( powen_layouts_id(), $rebuild );
        ...
        delete_option( powen_layouts_id() );
      ...
      /* redirect */
      if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == apply_filters( 'powen_theme_options_menu_slug', 'powen-theme-options' ) ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_alert_message' ) ) {

  function powen_alert_message( $page = array() ) {
    
    if ( empty( $page ) )
      return false;
    
    $before = apply_filters( 'powen_before_page_messages', '', $page );
    ...
    do_action( 'powen_custom_page_messages', $page );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_option_types_array' ) ) {

  function powen_option_types_array() {
  
    return apply_filters( 'powen_option_types_array', array( 
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_map_old_option_types' ) ) {

  function powen_map_old_option_types( $type = '' ) {
 ...
 * @since     2.5.0
 */
function powen_google_font_stack( $families, $field_id ) {

  $powen_google_fonts = get_theme_mod( 'powen_google_fonts', array() );
  $powen_set_google_fonts = get_theme_mod( 'powen_set_google_fonts', array() );

  if ( ! empty( $powen_set_google_fonts ) ) {
    foreach( $powen_set_google_fonts as $id => $sets ) {
      foreach( $sets as $value ) {
        $family = isset( $value['family'] ) ? $value['family'] : '';
        if ( $family && isset( $powen_google_fonts[$family] ) ) {
          $spaces = explode(' ', $powen_google_fonts[$family]['family'] );
          $font_stack = count( $spaces ) > 1 ? '"' . $powen_google_fonts[$family]['family'] . '"': $powen_google_fonts[$family]['family'];
          $families[$family] = apply_filters( 'powen_google_font_stack', $font_stack, $family, $field_id );
        }
      }
    }
  }
...
add_filter( 'powen_recognized_font_families', 'powen_google_font_stack', 1, 2 );
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_font_families' ) ) {

  function powen_recognized_font_families( $field_id = '' ) {
    ...
    return apply_filters( 'powen_recognized_font_families', $families, $field_id );
 ...
 * @since     2.0.12
 */
if ( ! function_exists( 'powen_recognized_font_sizes' ) ) {

  function powen_recognized_font_sizes( $field_id ) {
  
    $range = powen_range( 
      apply_filters( 'powen_font_size_low_range', 0, $field_id ), 
      apply_filters( 'powen_font_size_high_range', 150, $field_id ), 
      apply_filters( 'powen_font_size_range_interval', 1, $field_id )
    );
    
    $unit = apply_filters( 'powen_font_size_unit_type', 'px', $field_id );
    
    foreach( $range as $k => $v ) {
      $range[$k] = $v . $unit;
    }
    
    return apply_filters( 'powen_recognized_font_sizes', $range, $field_id );
  }
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_font_styles' ) ) {

  function powen_recognized_font_styles( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_font_styles', array(
      'normal'  => 'Normal',
      'italic'  => 'Italic',
      'oblique' => 'Oblique',
      'inherit' => 'Inherit'
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_font_variants' ) ) {

  function powen_recognized_font_variants( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_font_variants', array(
      'normal'      => 'Normal',
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_font_weights' ) ) {

  function powen_recognized_font_weights( $field_id = '' ) {
    
    return apply_filters( 'powen_recognized_font_weights', array(
      'normal'    => 'Normal',
      'bold'      => 'Bold',
      'bolder'    => 'Bolder',
      'lighter'   => 'Lighter',
 ...
 * @since     2.0.12
 */
if ( ! function_exists( 'powen_recognized_letter_spacing' ) ) {

  function powen_recognized_letter_spacing( $field_id ) {
  
    $range = powen_range( 
      apply_filters( 'powen_letter_spacing_low_range', -0.1, $field_id ), 
      apply_filters( 'powen_letter_spacing_high_range', 0.1, $field_id ), 
      apply_filters( 'powen_letter_spacing_range_interval', 0.01, $field_id )
    );
    
    $unit = apply_filters( 'powen_letter_spacing_unit_type', 'em', $field_id );
    
    foreach( $range as $k => $v ) {
      $range[$k] = $v . $unit;
    }
    
    return apply_filters( 'powen_recognized_letter_spacing', $range, $field_id );
  }
 ...
 * @since     2.0.12
 */
if ( ! function_exists( 'powen_recognized_line_heights' ) ) {

  function powen_recognized_line_heights( $field_id ) {
  
    $range = powen_range( 
      apply_filters( 'powen_line_height_low_range', 0, $field_id ), 
      apply_filters( 'powen_line_height_high_range', 150, $field_id ), 
      apply_filters( 'powen_line_height_range_interval', 1, $field_id )
    );
    
    $unit = apply_filters( 'powen_line_height_unit_type', 'px', $field_id );
    
    foreach( $range as $k => $v ) {
      $range[$k] = $v . $unit;
    }
    
    return apply_filters( 'powen_recognized_line_heights', $range, $field_id );
  }
 ...
 * @since     2.0.10
 */
if ( ! function_exists( 'powen_recognized_text_decorations' ) ) {
  
  function powen_recognized_text_decorations( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_text_decorations', array(
      'blink'         => 'Blink',
      'inherit'       => 'Inherit',
 ...
 * @since     2.0.10
 */
if ( ! function_exists( 'powen_recognized_text_transformations' ) ) {
  
  function powen_recognized_text_transformations( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_text_transformations', array(
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_background_repeat' ) ) {
  
  function powen_recognized_background_repeat( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_background_repeat', array(
      'no-repeat' => 'No Repeat',
      'repeat'    => 'Repeat All',
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_background_attachment' ) ) {

  function powen_recognized_background_attachment( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_background_attachment', array(
      "fixed"   => "Fixed",
      "scroll"  => "Scroll",
      "inherit" => "Inherit"
    ), $field_id );
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_recognized_background_position' ) ) {

  function powen_recognized_background_position( $field_id = '' ) {
  
    return apply_filters( 'powen_recognized_background_position', array(
      "left top"      => "Left Top",
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_border_style_types' ) ) {

  function powen_recognized_border_style_types( $field_id = '' ) {

    return apply_filters( 'powen_recognized_border_style_types', array(
      'hidden' => 'Hidden',
      'dashed' => 'Dashed',
      'solid'  => 'Solid',
      'double' => 'Double',
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_border_unit_types' ) ) {

  function powen_recognized_border_unit_types( $field_id = '' ) {

    return apply_filters( 'powen_recognized_border_unit_types', array(
      'px' => 'px',
      '%'  => '%',
      'em' => 'em',
      'pt' => 'pt'
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_dimension_unit_types' ) ) {

  function powen_recognized_dimension_unit_types( $field_id = '' ) {

    return apply_filters( 'powen_recognized_dimension_unit_types', array(
      'px' => 'px',
      '%'  => '%',
      'em' => 'em',
      'pt' => 'pt'
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_spacing_unit_types' ) ) {

  function powen_recognized_spacing_unit_types( $field_id = '' ) {

    return apply_filters( 'powen_recognized_spacing_unit_types', array(
      'px' => 'px',
      '%'  => '%',
      'em' => 'em',
      'pt' => 'pt'
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_google_font_families' ) ) {

  function powen_recognized_google_font_families( $field_id ) {

    $families = array();
    $powen_google_fonts = get_theme_mod( 'powen_google_fonts', array() );
    
    // Forces an array rebuild when we sitch themes
    if ( empty( $powen_google_fonts ) ) {
      $powen_google_fonts = powen_fetch_google_fonts( true, true );
    }
    
    foreach( (array) $powen_google_fonts as $key => $item ) {
    ...
    return apply_filters( 'powen_recognized_google_font_families', $families, $field_id );
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_google_font_variants' ) ) {

  function powen_recognized_google_font_variants( $field_id, $family ) {

    $variants = array();
    $powen_google_fonts = get_theme_mod( 'powen_google_fonts', array() );

    if ( isset( $powen_google_fonts[ $family ]['variants'] ) ) {
  
      $variants = $powen_google_fonts[ $family ]['variants'];
  
    }
  
    return apply_filters( 'powen_recognized_google_font_variants', $variants, $field_id, $family );
 ...
 * @since     2.5.0
 */
if ( ! function_exists( 'powen_recognized_google_font_subsets' ) ) {

  function powen_recognized_google_font_subsets( $field_id, $family ) {

    $subsets = array();
    $powen_google_fonts = get_theme_mod( 'powen_google_fonts', array() );
  
    if ( isset( $powen_google_fonts[ $family ]['subsets'] ) ) {
  
      $subsets = $powen_google_fonts[ $family ]['subsets'];
  
    }
  
    return apply_filters( 'powen_recognized_google_font_subsets', $subsets, $field_id, $family );
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_measurement_unit_types' ) ) {
  
  function powen_measurement_unit_types( $field_id = '' ) {
  
    return apply_filters( 'powen_measurement_unit_types', array(
      'px' => 'px',
      '%'  => '%',
      'em' => 'em',
      'pt' => 'pt'
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_radio_images' ) ) {
  
  function powen_radio_images( $field_id = '' ) {
  
    return apply_filters( 'powen_radio_images', array(
      array(
        'value'   => 'left-sidebar',
        'label'   => __( 'Left Sidebar', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/left-sidebar.png'
      ),
      array(
        'value'   => 'right-sidebar',
        'label'   => __( 'Right Sidebar', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/right-sidebar.png'
      ),
      array(
        'value'   => 'full-width',
        'label'   => __( 'Full Width (no sidebar)', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/full-width.png'
      ),
      array(
        'value'   => 'dual-sidebar',
        'label'   => __( 'Dual Sidebar', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/dual-sidebar.png'
      ),
      array(
        'value'   => 'left-dual-sidebar',
        'label'   => __( 'Left Dual Sidebar', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/left-dual-sidebar.png'
      ),
      array(
        'value'   => 'right-dual-sidebar',
        'label'   => __( 'Right Dual Sidebar', 'option-tree' ),
        'src'     => powen_URL . 'assets/images/layout/right-dual-sidebar.png'
      )
    ), $field_id );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_list_item_settings' ) ) {

  function powen_list_item_settings( $id ) {
    
    $settings = apply_filters( 'powen_list_item_settings', array(
      array(
        'id'        => 'image',
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_slider_settings' ) ) {

  function powen_slider_settings( $id ) {
 ...
 * @since     2.4.0
 */
if ( ! function_exists( 'powen_social_links_settings' ) ) {

  function powen_social_links_settings( $id ) {
    
    $settings = apply_filters( 'powen_social_links_settings', array(
      array(
        'id'        => 'name',
 ...
 * @updated   2.0.12
 */
if ( ! function_exists( 'powen_insert_css_with_markers' ) ) {

  function powen_insert_css_with_markers( $field_id = '', $insertion = '', $meta = false ) {
    ...
    $powen_css_file_paths = get_option( 'powen_css_file_paths', array() );
    if ( is_multisite() ) {
      $powen_css_file_paths = get_blog_option( get_current_blog_id(), 'powen_css_file_paths', $powen_css_file_paths );
    }

    /* set the path for this field */
    $powen_css_file_paths[$field_id] = $filepath;

    /* update the paths */
    if ( is_multisite() ) {
      update_blog_option( get_current_blog_id(), 'powen_css_file_paths', $powen_css_file_paths );
    } else {
      update_option( 'powen_css_file_paths', $powen_css_file_paths );
    }

    /* insert CSS into file */
    if ( file_exists( $filepath ) ) {
      
      $insertion   = powen_normalize_css( $insertion );
      $regex       = "/{{([a-zA-Z0-9\_\-\#\|\=]+)}}/";
      $marker      = $field_id;
      ...
      foreach( $matches[0] as $option ) {
        ...
        $option_type  = powen_get_option_type_by_id( $option_id );
        ...
        if ( is_array( $value ) ) {
          ...
          if ( ! isset( $option_array[1] ) ) {
            ...
            /* Border */
            } else if ( powen_array_keys_exists( $value, array( 'width', 'unit', 'style', 'color' ) ) && ! powen_array_keys_exists( $value, array( 'top', 'right', 'bottom', 'left', 'height', 'inset', 'offset-x', 'offset-y', 'blur-radius', 'spread-radius' ) ) ) {
              $border = array();
            ...
            /* Box Shadow */
            } else if ( powen_array_keys_exists( $value, array( 'inset', 'offset-x', 'offset-y', 'blur-radius', 'spread-radius', 'color' ) ) && ! powen_array_keys_exists( $value, array( 'width', 'height', 'unit', 'style', 'top', 'right', 'bottom', 'left' ) ) ) {

              /* set $value with box-shadow properties or empty string */
              $value = ! empty( $value ) ? implode( ' ', $value ) : '';
             
            /* Dimension */
            } else if ( powen_array_keys_exists( $value, array( 'width', 'height', 'unit' ) ) && ! powen_array_keys_exists( $value, array( 'style', 'color', 'top', 'right', 'bottom', 'left' ) ) ) {
              $dimension = array();
            ...
            /* Spacing */
            } else if ( powen_array_keys_exists( $value, array( 'top', 'right', 'bottom', 'left', 'unit' ) ) && ! powen_array_keys_exists( $value, array( 'width', 'height', 'style', 'color' ) ) ) {
              $spacing = array();
            ...
            /* typography */
            } else if ( powen_array_keys_exists( $value, array( 'font-color', 'font-family', 'font-size', 'font-style', 'font-variant', 'font-weight', 'letter-spacing', 'line-height', 'text-decoration', 'text-transform' ) ) ) {
              $font = array();
              ...
              if ( ! empty( $value['font-family'] ) ) {
                foreach ( powen_recognized_font_families( $marker ) as $key => $v ) {
            ...
            /* background */
            } else if ( powen_array_keys_exists( $value, array( 'background-color', 'background-image', 'background-repeat', 'background-attachment', 'background-position', 'background-size' ) ) ) {
              $bg = array();
              ...
              if ( isset( $size ) ) {
                if ( ! empty( $bg ) ) {
                  $value.= apply_filters( 'powen_insert_css_with_markers_bg_size_white_space', "\n\x20\x20", $option_id );
                }
        ...
        // Filter the CSS
        $value = apply_filters( 'powen_insert_css_with_markers_value', $value, $option_id );
      ...
      if ( ! $f = powen_file_open( $filepath, 'w' ) ) {
        return false;
      }
      ...
      if ( ! empty( $markerdata ) ) {
        ...
        foreach( $markerdata as $n => $markerline ) {
          ...
          if ( $markerline == "/* END {$marker} */" ) {
            powen_file_write( $f, "/* BEGIN {$marker} */\n" );
            powen_file_write( $f, "{$insertion}\n" );
            powen_file_write( $f, "/* END {$marker} */\n" );
            $searching = true;
            $foundit = true;
          }
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_remove_old_css' ) ) {

  function powen_remove_old_css( $field_id = '' ) {
 ...
 * @updated   2.0
 */
if ( ! function_exists( 'powen_normalize_css' ) ) {

  function powen_normalize_css( $css ) {
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_loop_through_option_types' ) ) {

  function powen_loop_through_option_types( $type = '', $child = false ) {
  
    $content = '';
    $types = powen_option_types_array();
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_loop_through_choices' ) ) {

  function powen_loop_through_choices( $name, $choices = array() ) {
    
    $content = '';
    
    foreach( (array) $choices as $key => $choice )
      $content.= '<li class="ui-state-default list-choice">' . powen_choices_view( $name, $key, $choice ) . '</li>';
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_loop_through_sub_settings' ) ) {

  function powen_loop_through_sub_settings( $name, $settings = array() ) {
    
    $content = '';
    
    foreach( $settings as $key => $setting )
      $content.= '<li class="ui-state-default list-sub-setting">' . powen_settings_view( $name, $key, $setting ) . '</li>';
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_sections_view' ) ) {

  function powen_sections_view( $name, $key, $section = array() ) {
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_settings_view' ) ) {

  function powen_settings_view( $name, $key, $setting = array() ) {
      ...
      <div class="option-tree-setting-body">
        ...
        <div class="format-settings">
          <div class="format-setting type-select wide-desc">
            ...
            <div class="format-setting-inner">
              ...
              ' . powen_loop_through_option_types( $type, $child ) . '                     
          ...
          <div class="format-setting type-textblock wide-desc">
            ...
            <div class="format-setting-inner">
              <ul class="option-tree-setting-wrap option-tree-sortable" data-name="' . esc_attr( $name ) . '[' . esc_attr( $key ) . ']">
                ' . ( isset( $setting['choices'] ) ? powen_loop_through_choices( $name . '[' . $key . ']', $setting['choices'] ) : '' ) . '
              </ul>
        ...
        <div class="format-settings">
          <div class="format-setting type-textblock wide-desc">
            ...
            <div class="format-setting-inner">
              <ul class="option-tree-setting-wrap option-tree-sortable" data-name="' . esc_attr( $name ) . '[' . esc_attr( $key ) . ']">
                ' . ( isset( $setting['settings'] ) ? powen_loop_through_sub_settings( $name . '[' . $key . '][settings]', $setting['settings'] ) : '' ) . '
              </ul>
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_choices_view' ) ) {

  function powen_choices_view( $name, $key, $choice = array() ) {
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_contextual_help_view' ) ) {

  function powen_contextual_help_view( $name, $key, $content = array() ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_layout_view' ) ) {

  function powen_layout_view( $key, $data = '', $active_layout = '' ) {
    ...
    <div class="option-tree-setting">
      ...
      <input type="hidden" name="' . powen_layouts_id() . '[' . esc_attr( $key ) . ']" value="' . $data . '" />
    </div>';
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_list_item_view' ) ) {

  function powen_list_item_view( $name, $key, $list_item = array(), $post_id = 0, $get_option = '', $settings = array(), $type = '' ) {
    ...
    if ( 'slider' == $type ) {
      ...
      $settings = powen_slider_settings( $name );
    ...
    if ( empty( $settings ) ) {
      ...
      $settings = powen_list_item_settings( $name );
    ...
    <div class="option-tree-setting">
      ...
      foreach( $settings as $field ) {
        ...
        if ( isset( $field['std'] ) ) {  
          $field_value = powen_filter_std_value( $field_value, $field['std'] );
        }
        ...
        if ( $field['id'] == 'title' ) {
          ...
          // filter the label
          $field['label'] = apply_filters( 'powen_list_item_title_label', $field['label'], $name );
          
          // filter the description
          $field['desc'] = apply_filters( 'powen_list_item_title_desc', $field['desc'], $name );
        ...
        echo '<div id="setting_' . $_args['field_id'] . '" class="' . $class . '"' . $conditions . '>';
          ...
          if ( apply_filters( 'powen_override_forced_textarea_simple', false, $field['id'] ) == false && $_args['type'] == 'textarea' )
            $_args['type'] = 'textarea-simple';
            
          /* option body, list-item is not allowed inside another list-item */
          if ( $_args['type'] !== 'list-item' && $_args['type'] !== 'slider' ) {
            echo powen_display_by_type( $_args );
          }
 ...
 * @since     2.4.0
 */
if ( ! function_exists( 'powen_social_links_view' ) ) {

  function powen_social_links_view( $name, $key, $list_item = array(), $post_id = 0, $get_option = '', $settings = array(), $type = '' ) {
    
    /* if no settings array load the filterable social links settings */
    if ( empty( $settings ) ) {
      
      $settings = powen_social_links_settings( $name );
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_theme_options_layouts_form' ) ) {

  function powen_theme_options_layouts_form( $active = false ) {
    ...
    echo '<form method="post" id="option-tree-options-layouts-form">';
      ...
      $layouts = get_option( powen_layouts_id() );
      ...
      if ( is_array( $layouts ) && count( $layouts ) > 1 ) {
        ...
        echo '<div class="option-tree-active-layout">';
          ...
          echo '<select name="' . powen_layouts_id() . '[active_layout]" class="option-tree-ui-select">';
        ...
        foreach( $layouts as $key => $data ) {
          ...
          echo '<input type="hidden" name="' . powen_layouts_id() . '[' . $key . ']" value="' . ( isset( $data ) ? $data : '' ) . '" />';
      ...
      echo '<div class="option-tree-save-layout' . ( ! empty( $active_layout ) ? ' active-layout' : '' ) . '">';
        ...
        /* add new layout */
        echo '<input type="text" name="' . powen_layouts_id() . '[_add_new_layout_]" value="" class="widefat option-tree-ui-input" autocomplete="off" />';
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_sanitize_option_id' ) ) {

  function powen_sanitize_option_id( $input ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_sanitize_layout_id' ) ) {

  function powen_sanitize_layout_id( $input ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_convert_array_to_string' ) ) {

  function powen_convert_array_to_string( $input ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_convert_string_to_array' ) ) {

  function powen_convert_string_to_array( $input ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_strpos_array' ) ) {

  function powen_strpos_array( $haystack, $needles = array() ) {
 ...
 * @since     2.0
 */
if ( ! function_exists( 'powen_array_keys_exists' ) ) {
  
  function powen_array_keys_exists( $array, $keys ) {
 ...
 * @since       2.0
 */
if ( ! function_exists( 'powen_stripslashes' ) ) {

  function powen_stripslashes( $input ) {
 ...
 * @since     2.0.9
 */
if ( ! function_exists( 'powen_reverse_wpautop' ) ) {

  function powen_reverse_wpautop( $string = '' ) {
 ...
 * @since     2.0.12
 */
function powen_range( $start, $limit, $step = 1 ) {
 ...
 * @since     2.0.13
 */
function powen_file_open( $handle, $mode ) {
 ...
 * @since     2.0.13
 */
function powen_file_close( $handle ) {
 ...
 * @since     2.0.13
 */
function powen_file_write( $handle, $string ) {
 ...
 * @since     2.0.15
 */
function powen_filter_std_value( $value = '', $std = '' ) {
 ...
 * @since     2.5.0
 */
function powen_set_google_fonts( $id = '', $value = '' ) {

  $powen_set_google_fonts = get_theme_mod( 'powen_set_google_fonts', array() );

  if ( is_array( $value ) && ! empty( $value ) ) {
    $powen_set_google_fonts[$id] = $value;
  } else if ( isset( $powen_set_google_fonts[$id] ) ) {
    unset( $powen_set_google_fonts[$id] );
  }

  set_theme_mod( 'powen_set_google_fonts', $powen_set_google_fonts );
 ...
 * @since     2.5.0
 */
function powen_update_google_fonts_after_save( $options ) {

  $powen_set_google_fonts = get_theme_mod( 'powen_set_google_fonts', array() );

  foreach( $powen_set_google_fonts as $key => $set ) {
    if ( ! isset( $options[$key] ) ) {
      unset( $powen_set_google_fonts[$key] );
    }
  }
  set_theme_mod( 'powen_set_google_fonts', $powen_set_google_fonts );

}
add_action( 'powen_after_theme_options_save', 'powen_update_google_fonts_after_save', 1 );
 ...
 * @since     2.5.0
 */
function powen_fetch_google_fonts( $normalize = true, $force_rebuild = false ) {

  /* Google Fonts cache key */
  $powen_google_fonts_cache_key = apply_filters( 'powen_google_fonts_cache_key', 'powen_google_fonts_cache' );

  /* get the fonts from cache */
  $powen_google_fonts = apply_filters( 'powen_google_fonts_cache', get_transient( $powen_google_fonts_cache_key ) );

  if ( $force_rebuild || ! is_array( $powen_google_fonts ) || empty( $powen_google_fonts ) ) {

    $powen_google_fonts = array();

    /* API url and key */
    $powen_google_fonts_api_url = apply_filters( 'powen_google_fonts_api_url', 'https://www.googleapis.com/webfonts/v1/webfonts' );
    $powen_google_fonts_api_key = apply_filters( 'powen_google_fonts_api_key', 'AIzaSyB8G-4UtQr9fhDYTiNrDP40Y5GYQQKrNWI' );

    /* API arguments */
    $powen_google_fonts_fields = apply_filters( 'powen_google_fonts_fields', array( 'family', 'variants', 'subsets' ) );
    $powen_google_fonts_sort   = apply_filters( 'powen_google_fonts_sort', 'alpha' );

    /* Initiate API request */
    $powen_google_fonts_query_args = array(
      'key'    => $powen_google_fonts_api_key, 
      'fields' => 'items(' . implode( ',', $powen_google_fonts_fields ) . ')', 
      'sort'   => $powen_google_fonts_sort
    );

    /* Build and make the request */
    $powen_google_fonts_query = add_query_arg( $powen_google_fonts_query_args, $powen_google_fonts_api_url );
    $powen_google_fonts_response = wp_safe_remote_get( $powen_google_fonts_query, array( 'sslverify' => false, 'timeout' => 15 ) );

    /* continue if we got a valid response */
    if ( 200 == wp_remote_retrieve_response_code( $powen_google_fonts_response ) ) {

      if ( $response_body = wp_remote_retrieve_body( $powen_google_fonts_response ) ) {

        /* JSON decode the response body and cache the result */
        $powen_google_fonts_data = json_decode( trim( $response_body ), true );

        if ( is_array( $powen_google_fonts_data ) && isset( $powen_google_fonts_data['items'] ) ) {

          $powen_google_fonts = $powen_google_fonts_data['items'];
          
          // Normalize the array key
          $powen_google_fonts_tmp = array();
          foreach( $powen_google_fonts as $key => $value ) {
            $id = remove_accents( $value['family'] );
            $id = strtolower( $id );
            $id = preg_replace( '/[^a-z0-9_\-]/', '', $id );
            $powen_google_fonts_tmp[$id] = $value;
          }
          
          $powen_google_fonts = $powen_google_fonts_tmp;
          set_theme_mod( 'powen_google_fonts', $powen_google_fonts );
          set_transient( $powen_google_fonts_cache_key, $powen_google_fonts, WEEK_IN_SECONDS );
  ...
  return $normalize ? powen_normalize_google_fonts( $powen_google_fonts ) : $powen_google_fonts;
 ...
 * @since     2.5.0
 */
function powen_normalize_google_fonts( $google_fonts ) {

  $powen_normalized_google_fonts = array();
  ...
  if ( is_array( $google_fonts ) && ! empty( $google_fonts ) ) {
    ...
    foreach( $google_fonts as $google_font ) {
      ...
      if( isset( $google_font['family'] ) ) {
        ...
        $powen_normalized_google_fonts[ $id ] = array(
          'family' => $google_font['family']
        );

        if( isset( $google_font['variants'] ) ) {

          $powen_normalized_google_fonts[ $id ]['variants'] = $google_font['variants'];

        }

        if( isset( $google_font['subsets'] ) ) {

          $powen_normalized_google_fonts[ $id ]['subsets'] = $google_font['subsets'];
  ...
  return $powen_normalized_google_fonts;
 ...
 * @since     2.1
 */
function powen_wpml_register_string( $id, $value ) {
 ...
 * @since     2.1
 */
function powen_wpml_unregister_string( $id ) {
 ...
 * @since     2.3.3
 */
if ( ! function_exists( 'powen_maybe_migrate_settings' ) ) {

  function powen_maybe_migrate_settings() {
    
    // Filter the ID to migrate from
    $settings_id = apply_filters( 'powen_migrate_settings_id', '' );
    
    // Attempt to migrate Settings 
    if ( ! empty( $settings_id ) && get_option( powen_settings_id() ) === false && powen_settings_id() !== $settings_id ) {
 ...
 * @since     2.3.3
 */
if ( ! function_exists( 'powen_maybe_migrate_options' ) ) {

  function powen_maybe_migrate_options() {
    
    // Filter the ID to migrate from
    $options_id = apply_filters( 'powen_migrate_options_id', '' );
    
    // Attempt to migrate Theme Options
    if ( ! empty( $options_id ) && get_option( powen_options_id() ) === false && powen_options_id() !== $options_id ) {
 ...
 * @since     2.3.3
 */
if ( ! function_exists( 'powen_maybe_migrate_layouts' ) ) {

  function powen_maybe_migrate_layouts() {
    
    // Filter the ID to migrate from
    $layouts_id = apply_filters( 'powen_migrate_layouts_id', '' );
    
    // Attempt to migrate Layouts
    if ( ! empty( $layouts_id ) && get_option( powen_layouts_id() ) === false && powen_layouts_id() !== $layouts_id ) {
 ...
 * @since     2.4.0
 */
function powen_meta_box_post_format_gallery( $pages = 'post' ) {
  ...
  return apply_filters( 'powen_meta_box_post_format_gallery', array(
 ...
 * @since     2.4.0
 */
function powen_meta_box_post_format_link( $pages = 'post' ) {
  ...
  return apply_filters( 'powen_meta_box_post_format_link', array(
 ...
 * @since     2.4.0
 */
function powen_meta_box_post_format_quote( $pages = 'post' ) {
  ...
  return apply_filters( 'powen_meta_box_post_format_quote', array(
 ...
 * @since     2.4.0
 */
function powen_meta_box_post_format_video( $pages = 'post' ) {
  ...
  return apply_filters( 'powen_meta_box_post_format_video', array(
 ...
 * @since     2.4.0
 */
function powen_meta_box_post_format_audio( $pages = 'post' ) {
  ...
  return apply_filters( 'powen_meta_box_post_format_audio', array(
 ...
 * @since     2.4.2
 */
if ( ! function_exists( 'powen_get_option_type_by_id' ) ) {

  function powen_get_option_type_by_id( $option_id, $settings_id = '' ) {
    
    if ( empty( $settings_id ) ) {
    
      $settings_id = powen_settings_id();
