 ...
 */
if ( ! class_exists( 'powen_Meta_Box' ) ) {

  class powen_Meta_Box {
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
        ...
        if ( isset( $_POST[$field['id']] ) ) {
          ...
          if ( in_array( $field['type'], array( 'list-item', 'slider' ) ) ) {
            ...
            $settings = isset( $_POST[$field['id'] . '_settings_array'] ) ? unserialize( powen_decode( $_POST[$field['id'] . '_settings_array'] ) ) : array();
            
            /* settings are empty for some odd ass reason get the defaults */
            if ( empty( $settings ) ) {
              $settings = 'slider' == $field['type'] ? 
              powen_slider_settings( $field['id'] ) : 
              powen_list_item_settings( $field['id'] );
            }
            ...
            foreach( $_POST[$field['id']] as $k => $setting_array ) {
              ...
              foreach( $settings as $sub_setting ) {
                ...
                if ( isset( $sub_setting['type'] ) && isset( $_POST[$field['id']][$k][$sub_setting['id']] ) ) {
                  ...
                  $_POST[$field['id']][$k][$sub_setting['id']] = powen_validate_setting( $_POST[$field['id']][$k][$sub_setting['id']], $sub_setting['type'], $sub_setting['id'] );
            ...
            if ( empty( $settings ) ) {
              $settings = powen_social_links_settings( $field['id'] );
            }
            ...
            $new = powen_validate_setting( $_POST[$field['id']], $field['type'], $field['id'] );
          ...
          if ( $field['type'] == 'css' ) {
            ...
            if ( '' !== $new ) {
              ...
              powen_insert_css_with_markers( $field['id'], $new, true );
            
            /* remove old CSS from dynamic.css */
            } else {
            
              powen_remove_old_css( $field['id'] );
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_register_meta_box' ) ) {

  function powen_register_meta_box( $args ) {
    if ( ! $args )
      return;
      
    $powen_meta_box = new powen_Meta_Box( $args );
  }
