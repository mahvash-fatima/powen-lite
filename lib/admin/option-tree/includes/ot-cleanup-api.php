...
if ( ! class_exists( 'powen_Cleanup' ) ) {
  ...
  class powen_Cleanup {
    ...
    function __construct() {
      ...
      add_action( 'powen_pre_consolidate_posts', array( $this, 'increase_timeout' ) );
    ...
    public function maybe_cleanup() {
      global $wpdb, $table_prefix, $powen_maybe_cleanup_posts, $powen_maybe_cleanup_table;
      ...
      $powen_maybe_cleanup_posts = count( $posts ) > 1;
      $powen_maybe_cleanup_table = count( $table ) == 1;
      $page = isset( $_GET['page'] ) ? $_GET['page'] : '';

      if ( ! $powen_maybe_cleanup_posts && ! $powen_maybe_cleanup_table && $page == 'ot-cleanup' ) {
        wp_redirect( apply_filters( 'powen_theme_options_parent_slug', 'themes.php' ) . '?page=' . apply_filters( 'powen_theme_options_menu_slug', 'powen-theme-options' ) );
        exit;
      }

      if ( $powen_maybe_cleanup_posts || $powen_maybe_cleanup_table ) {
        ...
        $theme_check_bs( apply_filters( 'powen_cleanup_page_title', __( 'OptionTree Cleanup', 'option-tree' ) ), apply_filters( 'powen_cleanup_menu_title', __( 'OptionTree Cleanup', 'option-tree' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( __( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'option-tree' ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'powen_cleanup_menu_title', __( 'OptionTree Cleanup', 'option-tree' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        echo '<h2>' . apply_filters( 'powen_cleanup_page_title', __( 'OptionTree Cleanup', 'option-tree' ) ) . '</h2>';

        if ( $powen_maybe_cleanup_posts ) {
          ...
          echo $safe_mode ?  '<p>' . sprintf( __( '%s Your server is running in safe mode. Which means this page will automatically reload after deleting %s posts, you can filter this number using %s if your server is having trouble processing that many at one time.', 'option-tree' ), '<strong>Note</strong>:', apply_filters( 'powen_consolidate_posts_reload', 500 ), '<tt>powen_consolidate_posts_reload</tt>' ) . '</p>' : '';
          ...
          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'consolidate-posts' ) ) {
            ...
            do_action( 'powen_pre_consolidate_posts' );
            ...
            foreach( $posts as $post ) {
              ...
              if ( $safe_mode && $count > apply_filters( 'powen_consolidate_posts_reload', 500 ) ) {
                echo '<br />' . __( 'Reloading...', 'option-tree' );
                echo '
                <script>
                  setTimeout( powen_script_reload, 3000 )
                  function powen_script_reload() {
        ...
        if ( $powen_maybe_cleanup_table ) {

          $table_name = $table_prefix . 'option_tree';

          echo $powen_maybe_cleanup_posts ? '<hr />' : '';
