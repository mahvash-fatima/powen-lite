 ...
 */
if ( ! class_exists( 'powen_Settings' ) ) {

  class powen_Settings {
    ...
    public function hooks() {
       ...
       */
      $priority = apply_filters( 'powen_admin_menu_priority', 10 );
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( isset( $_REQUEST['settings-updated'] ) && $_REQUEST['settings-updated'] == 'true' ) {
              ...
              if ( isset( $layouts['active_layout'] ) ) {
                ...
                $layouts[$layouts['active_layout']] = powen_encode( serialize( $option_tree ) );
            ...
            echo '<div class="wrap settings-wrap" id ="page-' . $page['id'] . '">';
              ...
              echo '<div id="option-tree-header-wrap">';
                ...
                echo '<ul id="option-tree-header">';
                  ...
                  echo '<li id="option-tree-logo">' . apply_filters( 'powen_header_logo_link', '<a href="http://wordpress.org/extend/plugins/option-tree/" target="_blank">OptionTree</a>', $page['id'] ) . '</li>';
                  
                  echo '<li id="option-tree-version"><span>' . apply_filters( 'powen_header_version_text', 'OptionTree ' . powen_VERSION, $page['id'] ) . '</span></li>';
                  
                  // Add additional theme specific links here.
                  do_action( 'powen_header_list', $page['id'] );
                
                echo '</ul>';
                
                /* layouts form */
                if ( $page['id'] == 'powen_theme_options' && powen_SHOW_NEW_LAYOUT == true )
                  powen_theme_options_layouts_form();
    ...
    public function display_setting( $args = array() ) {
      ...
      /* set standard value */
      if ( isset( $std ) ) {  
        $field_value = powen_filter_std_value( $field_value, $std );
      }
      
      // Allow the descriptions to be filtered before being displayed
      $desc = apply_filters( 'powen_filter_description', ( isset( $desc ) ? $desc : '' ), $id );
      ...
      if ( $type == 'google-fonts' ) {
        ...
        powen_set_google_fonts( $id, $field_value );
      }
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          foreach( (array) $this->get_the_settings( $page ) as $setting ) {
            ...
            if ( isset( $setting['std'] ) ) {
              ...
              $defaults[$setting['id']] = powen_validate_setting( $setting['std'], $setting['type'], $setting['id'] );
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          foreach( (array) $this->get_the_settings( $page ) as $setting ) {
            ...
            if ( isset( $setting['type'] ) && isset( $input[$setting['id']] ) ) {
              ...
              /* get the defaults */
              $current_settings = get_option( powen_settings_id() );
              ...
              if ( is_array( $input[$setting['id']] ) && in_array( $setting['type'], array( 'list-item', 'slider' ) ) ) {
                ...
                $settings = isset( $_POST[$setting['id'] . '_settings_array'] ) ? unserialize( powen_decode( $_POST[$setting['id'] . '_settings_array'] ) ) : array();
                
                /* settings are empty for some odd ass reason get the defaults */
                if ( empty( $settings ) ) {
                  $settings = 'slider' == $setting['type'] ? 
                  powen_slider_settings( $setting['id'] ) : 
                  powen_list_item_settings( $setting['id'] );
                }
                ...
                foreach( $input[$setting['id']] as $k => $setting_array ) {
                  ...
                  foreach( $settings as $sub_setting ) {
                    ...
                    if ( isset( $sub_setting['type'] ) && isset( $input[$setting['id']][$k][$sub_setting['id']] ) ) {
                      ...
                      /* validate setting */
                      $input[$setting['id']][$k][$sub_setting['id']] = powen_validate_setting( $input[$setting['id']][$k][$sub_setting['id']], $sub_setting['type'], $sub_setting['id'], $wpml_id );
                ...
                if ( empty( $settings ) ) {
                  $settings = powen_social_links_settings( $setting['id'] );
                }
                ...
                $input[$setting['id']] = powen_validate_setting( $input[$setting['id']], $setting['type'], $setting['id'], $setting['id'] );
                  ...
                  foreach( $current_options[$setting['id']] as $key => $value ) {
                    ...
                    foreach( $value as $ckey => $cvalue ) {
                      ...
                      if ( ! in_array( $id, $wpml_ids ) ) {
                        ...
                        powen_wpml_unregister_string( $id );
 ...
 * @since    2.0
 */
if ( ! function_exists( 'powen_register_settings' ) ) {

  function powen_register_settings( $args ) {
    if ( ! $args )
      return;
      
    $powen_settings = new powen_Settings( $args );
  }
