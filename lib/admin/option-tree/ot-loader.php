 ...
 */
if ( class_exists( 'powen_Loader' ) && defined( 'powen_PLUGIN_MODE' ) && powen_PLUGIN_MODE == true ) {
  
  add_filter( 'powen_theme_mode', '__return_false', 999 );
  
  function powen_conflict_notice() {
  ...
  add_action( 'admin_notices', 'powen_conflict_notice' );
...
if ( ! class_exists( 'powen_Loader' ) ) {
  ...
  class powen_Loader {
    ...
    private function load_languages() {
       ...
       * @since     2.1.3
       */
      define( 'powen_PLUGIN_MODE', strpos( dirname( __FILE__ ), 'plugins' . DIRECTORY_SEPARATOR . basename( dirname( __FILE__ ) ) ) !== false ? true : false );
       ...
       * @updated   2.4.1
       */
      if ( powen_PLUGIN_MODE ) {
        
        define( 'powen_LANG_DIR', trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) . trailingslashit( 'languages' ) );
        
      } else {
      
        if ( apply_filters( 'powen_child_theme_mode', false ) == true ) {
        
          $path = ltrim( end( @explode( get_stylesheet(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'powen_LANG_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) . trailingslashit( 'languages' ) . 'theme-mode' );
          
        } else {
        
          $path = ltrim( end( @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'powen_LANG_DIR', trailingslashit( trailingslashit( get_template_directory() ) . $path ) . trailingslashit( 'languages' ) . 'theme-mode' );
    ...
    public function load_textdomain() {
      ...
      if ( powen_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'option-tree', false, powen_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'option-tree', powen_LANG_DIR );
     ...
     * are prefixed with 'powen_' to avoid any naming collisions.
     *
     * @return    void
     *
     * @access    private
    ...
    private function constants() {
       ...
       * @since     2.0
       */
      define( 'powen_THEME_MODE', apply_filters( 'powen_theme_mode', false ) );
       ...
       * @since     2.0.15
       */
      define( 'powen_CHILD_THEME_MODE', apply_filters( 'powen_child_theme_mode', false ) );
       ...
       * @since     2.0
       */
      define( 'powen_SHOW_PAGES', apply_filters( 'powen_show_pages', true ) );
       ...
       * @since     2.1
       */
      define( 'powen_SHOW_OPTIONS_UI', apply_filters( 'powen_show_options_ui', true ) );
       ...
       * @since     2.1
       */
      define( 'powen_SHOW_SETTINGS_IMPORT', apply_filters( 'powen_show_settings_import', true ) );
       ...
       * @since     2.1
       */
      define( 'powen_SHOW_SETTINGS_EXPORT', apply_filters( 'powen_show_settings_export', true ) );
       ...
       * @since     2.0.10
       */
      define( 'powen_SHOW_NEW_LAYOUT', apply_filters( 'powen_show_new_layout', true ) );
       ...
       * @since     2.1
       */
      define( 'powen_SHOW_DOCS', apply_filters( 'powen_show_docs', true ) );
       ...
       * @since     2.1
       */
      define( 'powen_USE_THEME_OPTIONS', apply_filters( 'powen_use_theme_options', true ) );
       ...
       * @since     2.0
       */
      define( 'powen_META_BOXES', apply_filters( 'powen_meta_boxes', true ) );
       ...
       * @since     2.0
       */
      define( 'powen_ALLOW_UNFILTERED_HTML', apply_filters( 'powen_allow_unfiltered_html', false ) );
       ...
       * @since     2.4.0
       */
      define( 'powen_POST_FORMATS', apply_filters( 'powen_post_formats', false ) );
      
      /**
       * Check if in theme mode.
       *
       * If powen_THEME_MODE and powen_CHILD_THEME_MODE is false, set the 
       * directory path & URL like any other plugin. Otherwise, use 
       * the parent or child themes root directory. 
       *
       * @since     2.0
       */
      if ( false == powen_THEME_MODE && false == powen_CHILD_THEME_MODE ) {
        define( 'powen_DIR', plugin_dir_path( __FILE__ ) );
        define( 'powen_URL', plugin_dir_url( __FILE__ ) );
      } else {
        if ( true == powen_CHILD_THEME_MODE ) {
          $path = ltrim( end( @explode( get_stylesheet(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'powen_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) );
          define( 'powen_URL', trailingslashit( trailingslashit( get_stylesheet_directory_uri() ) . $path ) );
        } else {
          $path = ltrim( end( @explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) ) ), '/' );
          define( 'powen_DIR', trailingslashit( trailingslashit( get_template_directory() ) . $path ) );
          define( 'powen_URL', trailingslashit( trailingslashit( get_template_directory_uri() ) . $path ) );
        }
      }
       ...
       */
      if ( true == powen_CHILD_THEME_MODE ) {
        define( 'powen_THEME_URL', get_stylesheet_directory_uri() );
      } else {
        define( 'powen_THEME_URL', get_template_directory_uri() );
      }
    ...
    private function admin_includes() {
      ...
      foreach ( $files as $file ) {
        $this->load_file( powen_DIR . "includes" . DIRECTORY_SEPARATOR . "{$file}.php" );
      }
      
      /* Registers the Theme Option page */
      add_action( 'init', 'powen_register_theme_options_page' );
      
      /* Registers the Settings page */
      if ( powen_SHOW_PAGES == true ) {
        add_action( 'init', 'powen_register_settings_page' );
      }
    ...
    private function hooks() {
      ...
      if ( function_exists( 'powen_maybe_migrate_settings' ) )
        add_action( 'init', 'powen_maybe_migrate_settings', 1 );
      
      // Attempt to migrate the Options
      if ( function_exists( 'powen_maybe_migrate_options' ) )
        add_action( 'init', 'powen_maybe_migrate_options', 1 );
      
      // Attempt to migrate the Layouts
      if ( function_exists( 'powen_maybe_migrate_layouts' ) )
        add_action( 'init', 'powen_maybe_migrate_layouts', 1 );

      /* load the Meta Box assets */
      if ( powen_META_BOXES == true ) {
      
        /* add scripts for metaboxes to post-new.php & post.php */
        add_action( 'admin_print_scripts-post-new.php', 'powen_admin_scripts', 11 );
        add_action( 'admin_print_scripts-post.php', 'powen_admin_scripts', 11 );
              
        /* add styles for metaboxes to post-new.php & post.php */
        add_action( 'admin_print_styles-post-new.php', 'powen_admin_styles', 11 );
        add_action( 'admin_print_styles-post.php', 'powen_admin_styles', 11 );
      
      }
      
      /* Adds the Theme Option page to the admin bar */
      add_action( 'admin_bar_menu', 'powen_register_theme_options_admin_bar_menu', 999 );
      
      /* prepares the after save do_action */
      add_action( 'admin_init', 'powen_after_theme_options_save', 1 );
      
      /* default settings */
      add_action( 'admin_init', 'powen_default_settings', 2 );
      
      /* add xml to upload filetypes array */
      add_action( 'admin_init', 'powen_add_xml_to_upload_filetypes', 3 );
      
      /* import */
      add_action( 'admin_init', 'powen_import', 4 );
      
      /* export */
      add_action( 'admin_init', 'powen_export', 5 );
      
      /* save settings */
      add_action( 'admin_init', 'powen_save_settings', 6 );
      
      /* save layouts */
      add_action( 'admin_init', 'powen_modify_layouts', 7 );
      
      /* create media post */
      add_action( 'admin_init', 'powen_create_media_post', 8 );
      
      /* global CSS */
      add_action( 'admin_head', array( $this, 'global_admin_css' ) );

      /* Google Fonts front-end CSS */
      add_action( 'wp_enqueue_scripts', 'powen_load_google_fonts_css', 1 );
 
      /* dynamic front-end CSS */
      add_action( 'wp_enqueue_scripts', 'powen_load_dynamic_css', 999 );

      /* insert theme CSS dynamically */
      add_action( 'powen_after_theme_options_save', 'powen_save_css' );
      ...
      add_action( 'wp_ajax_powen_google_font', array( $this, 'retrieve_google_font' ) );
    ...
    public function global_admin_css() {
      ...
      <style>
        @font-face {
          ...
          src:url("' . powen_URL . 'assets/fonts/option-tree-font.eot");
          src:url("' . powen_URL . 'assets/fonts/option-tree-font.eot?#iefix") format("embedded-opentype"),
            url("' . powen_URL . 'assets/fonts/option-tree-font.woff") format("woff"),
            url("' . powen_URL . 'assets/fonts/option-tree-font.ttf") format("truetype"),
            url("' . powen_URL . 'assets/fonts/option-tree-font.svg#option-tree-font") format("svg");
          font-weight: normal;
          font-style: normal;
        }
        #adminmenu #toplevel_page_ot-settings .menu-icon-generic div.wp-menu-image:before,
    ...
    public function add_section() {
      echo powen_sections_view( powen_settings_id() . '[sections]', $_REQUEST['count'] );
      die();
    }
    ...
    public function add_setting() {
      echo powen_settings_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_list_item_setting() {
      echo powen_settings_view( $_REQUEST['name'] . '[settings]', $_REQUEST['count'] );
      die();
    }
    ...
    public function add_the_contextual_help() {
      echo powen_contextual_help_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_choice() {
      echo powen_choices_view( $_REQUEST['name'], $_REQUEST['count'] );
      die();
    }
    ...
    public function add_layout() {
      echo powen_layout_view( $_REQUEST['count'] );
      die();
    }
    ...
    public function add_list_item() {
      powen_list_item_view( $_REQUEST['name'], $_REQUEST['count'], array(), $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize( powen_decode( $_REQUEST['settings'] ) ), $_REQUEST['type'] );
      die();
    }
    ...
    public function add_social_links() {
      powen_social_links_view( $_REQUEST['name'], $_REQUEST['count'], array(), $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize( powen_decode( $_REQUEST['settings'] ) ), $_REQUEST['type'] );
      die();
    }
    ...
    public function shortcode( $settings, $post ) {
      ...
      if ( ! is_object( $post ) ) {
        ...
        if ( $post_id == 0 && function_exists( 'powen_get_media_post_ID' ) ) {
      ...
      $settings['powen_gallery'] = array( 'shortcode' => "[gallery id='{$settings['post']['id']}']" );
    ...
    public function retrieve_google_font() {
      ...
      if ( isset( $_POST['field_id'], $_POST['family'] ) ) {
        ...
        powen_fetch_google_fonts();
        
        echo json_encode( array(
          'variants' => powen_recognized_google_font_variants( $_POST['field_id'], $_POST['family'] ),
          'subsets'  => powen_recognized_google_font_subsets( $_POST['field_id'], $_POST['family'] )
        ) );
        exit();
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'powen_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        
        // Once is enough.
        remove_filter( 'gettext', array( $this, 'powen_change_image_button' ) );
        return apply_filters( 'powen_upload_text', __( 'Send to OptionTree', 'option-tree' ) );
